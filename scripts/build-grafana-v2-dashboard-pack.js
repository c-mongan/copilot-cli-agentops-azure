...
function recommendationsNormalize() {
  return [
    ...
    "| extend ChangeAnnotations=column_ifexists('ChangeAnnotations', dynamic([]))",
...
function configAnnotationsQuery() {
  return [
    "union isfuzzy=true",
    "(AgentOpsEvents_CL | where TimeGenerated between ($__timeFrom() .. $__timeTo()) | extend EventName=tostring(column_ifexists('EventName', '')), EventType=tostring(column_ifexists('EventType', '')), Details=tostring(column_ifexists('Details', '')) | where EventName == 'agentops.config.changed' or EventType == 'annotation' or Details has 'config_change' | extend AnnotationType=coalesce(tostring(column_ifexists('AnnotationType', '')), extract('annotation_type[=: ]+([A-Za-z0-9_-]+)', 1, Details)), ChangeComponent=tostring(column_ifexists('ChangeComponent', '')), ChangeTarget=tostring(column_ifexists('ChangeTarget', '')), ChangeType=tostring(column_ifexists('ChangeType', '')), ChangeId=tostring(column_ifexists('ChangeId', '')), Version=tostring(column_ifexists('Version', '')), RunId=tostring(column_ifexists('RunId', '')), SessionId=tostring(column_ifexists('SessionId', '')), TraceId=tostring(column_ifexists('TraceId', '')), AgentName=tostring(column_ifexists('AgentName', '')), Risk=tostring(column_ifexists('Risk', '')), Source='AgentOpsEvents_CL' | project TimeGenerated, Source, EventName, AnnotationType, ChangeComponent, ChangeTarget, ChangeType, ChangeId, Version, AgentName, Risk, RunId, SessionId, TraceId, Details),",
    "(AppDependencies | where TimeGenerated between ($__timeFrom() .. $__timeTo()) | where tostring(Properties['agentops.event.name']) == 'agentops.config.changed' or tostring(Properties['agentops.custom.annotation_type']) == 'config_change' | extend EventName=tostring(Properties['agentops.event.name']), AnnotationType=tostring(Properties['agentops.custom.annotation_type']), ChangeComponent=tostring(Properties['agentops.custom.component']), ChangeTarget=tostring(Properties['agentops.custom.target']), ChangeType=tostring(Properties['agentops.custom.change_type']), ChangeId=tostring(Properties['agentops.custom.change_id']), Version=tostring(Properties['agentops.custom.version']), AgentName=tostring(Properties['agentops.agent.name']), Risk=tostring(Properties['agentops.risk']), RunId=tostring(Properties['agentops.run.id']), SessionId=tostring(Properties['gen_ai.conversation.id']), TraceId=tostring(Properties['agentops.trace.id']), Details='', Source='AppDependencies' | project TimeGenerated, Source, EventName, AnnotationType, ChangeComponent, ChangeTarget, ChangeType, ChangeId, Version, AgentName, Risk, RunId, SessionId, TraceId, Details)"
  ].join(' ');
}
