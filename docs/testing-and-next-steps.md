...
`alert tune-plan` is proposal-only and summarizes reviewable threshold changes with Bicep patch targets. `alert resources` is read-only and summarizes current scheduled-query rule enabled/disabled state plus attached action groups.
