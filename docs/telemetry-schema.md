...
The wrapper adds metadata-only envelope attributes when launching Copilot: `agentops.wrapper.run_id`, `agentops.wrapper.session_id`, and `agentops.wrapper.fallback_unobserved`. If the collector cannot start and `AGENTOPS_ALLOW_UNOBSERVED_FALLBACK=1` permits an unobserved run, the CLI also appends a local `agentops.wrapper.fallback_unobserved` JSONL row to `.agentops/wrapper-events.jsonl`.
